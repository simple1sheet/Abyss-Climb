...
export class QuestGenerator {
  ...
  private async generateAutomaticDailyQuests(userId: string, currentLayer: number, userSkills: any[]): Promise<void> {
    ...
    // Get existing daily quests for today (both active and completed)
    ...
    // Check if we already have 3 daily quests for today
    if (existingDailyQuests.length >= 3) {
      console.log(`User ${userId} already has ${existingDailyQuests.length} daily quests for today, skipping generation`);
      return;
    }

    // Generate exactly 3 daily quests total
    const questsToGenerate = 3 - existingDailyQuests.length;
