...
export class QuestGenerator {
  ...
  private async generateAutomaticDailyQuests(userId: string, currentLayer: number, userSkills: any[]): Promise<void> {
    // Get all active daily quests (not expired)
    ...
    // Check if we already have 3 active daily quests
    if (activeDailyQuests.length >= 3) {
      console.log(`User ${userId} already has ${activeDailyQuests.length} active daily quests, skipping generation`);
      return;
    }

    // Generate exactly 3 daily quests total
    const questsToGenerate = 3 - activeDailyQuests.length;
    console.log(`Generating ${questsToGenerate} daily quests for user ${userId}`);
    
    for (let i = 0; i < questsToGenerate; i++) {
      try {
        await this.generateSingleAutomaticDailyQuest(userId, currentLayer, userSkills, activeDailyQuests);
