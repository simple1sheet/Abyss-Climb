...
export class QuestGenerator {
  ...
  async generateAutomaticQuests(userId: string): Promise<void> {
    ...
    // First, clean up expired quests for this user
    await this.cleanupExpiredQuestsForUser(userId);
  ...
  private async generateAutomaticDailyQuests(userId: string, currentLayer: number, userSkills: any[]): Promise<void> {
    // Check if daily quests have already been generated today
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    const allUserQuests = await storage.getUserQuests(userId);
    const dailyQuestsToday = allUserQuests.filter(q => {
      if (q.questType !== 'daily' || !q.autoGenerated) return false;
      const questDate = new Date(q.createdAt);
      questDate.setHours(0, 0, 0, 0);
      return questDate.getTime() === today.getTime();
    });

    // If daily quests have already been generated today, don't generate more
    if (dailyQuestsToday.length >= 3) {
      console.log(`User ${userId} already has ${dailyQuestsToday.length} daily quests generated today, skipping generation`);
      return;
    }

    // Only generate quests if none have been generated today
    if (dailyQuestsToday.length === 0) {
      console.log(`Generating 3 daily quests for user ${userId}`);
      
      for (let i = 0; i < 3; i++) {
        try {
          await this.generateSingleAutomaticDailyQuest(userId, currentLayer, userSkills, dailyQuestsToday);
  ...
  private async generateAutomaticWeeklyQuest(userId: string, currentLayer: number, userSkills: any[]): Promise<void> {
    ...
    if (hasActiveWeeklyQuest) {
      console.log(`User ${userId} already has an active weekly quest, skipping generation`);
    ...
    if (hasRecentWeeklyQuest) {
      console.log(`User ${userId} recently completed a weekly quest, skipping generation`);
  ...
  // Method to clean up expired quests for a specific user
  async cleanupExpiredQuestsForUser(userId: string): Promise<void> {
    try {
      const now = new Date();
      const allUserQuests = await storage.getUserQuests(userId, "active");
      ...
      if (expiredCount > 0) {
        console.log(`Cleaned up ${expiredCount} expired quests for user ${userId}`);
      }
    } catch (error) {
      console.error(`Failed to cleanup expired quests for user ${userId}:`, error);
    }
  }

  // Method to clean up expired quests for all users
